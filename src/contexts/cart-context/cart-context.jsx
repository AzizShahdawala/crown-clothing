...
const removeCartItem = (cartItems, productToRemove) => {
  ...
  // If item not found, just return cart as is
  if (!existingCartItem) {
    return cartItems;
  }

  // If quantity is 1, remove it from cart entirely
...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    setTotalCartAmount(newTotalAmount);
  }, [cartItems]);
  ...
  const deleteCheckOutItem = (checkoutItem) => {
    setCartItems(
      cartItems.filter((cartItem) => cartItem.id !== checkoutItem.id)
    );
  };
